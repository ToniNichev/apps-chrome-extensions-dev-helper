# Privacy Policy — Dev Helper

_Last updated: 2026-08-18_

Dev Helper is a developer tool for profiling network requests, and configuring proxy, rewrite, mock, and script-injection rules while you browse. This policy explains what the extension does and does not do with your data.

## What Dev Helper does not do
...
## What Dev Helper stores, and where
...
| `webRequest` | Read request metadata (URL, method, timing, status) to power the Profiling tab. Used read-only; Dev Helper does not use `webRequest` to block or modify traffic. |
...
| `host_permissions: <all_urls>` | Dev Helper's whole purpose is letting you apply rules to any site you choose to debug — this permission lets your rules run on whichever site you point them at, rather than a fixed list. |

## Proxy rules and third-party servers

If you configure a proxy rule, Dev Helper routes matching traffic through the proxy server **you specify**. That server is chosen and controlled by you, not the developer — Dev Helper does not operate, route through, or have visibility into any proxy server of its own.
